...
export default function ItemList(props) {
    const {task, onChange, onClickDel} = props;
    const icDelete = { backgroundImage: `url(/ic-del.png)`, width:'32px', height:'32px' };
    return(
        <div className="list-item" id={task.id}>
            <input className="Radio" type="checkbox" onChange={onChange} checked={task.checked}/>            
            <span className="item-content-task">{task.text}</span>
            <span  style={icDelete} onClick={onClickDel}></span>
            <style jsx>{`
                .list-item {
                    height:56px;
                    display:flex;
